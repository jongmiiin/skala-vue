...
const handleFetchWeather = async () => {
  ...
  try {
    // 비동기 통신 가동: 서버에서 데이터를 다 가져올 때까지 await으로 대기
    const response = await axios.get(URL)

    //fetch와 달리 .json() 변환 과정 없이 response.data에 알맹이가 즉시 담긴다
    console.log('Axios 통신 응답 전체 객체:', response)
    console.log('백엔드가 준 핵심 날씨 데이터(JSON):', response.data)
    weatherData.value = response.data
  } catch (error) {
    // 4xx, 5xx 에러나 네트워크 오프라인 시 자동으로 이 catch 영역으로 튕겨온다
    console.error('통신 중 에러가 발생했습니다:', error)
    alert('데이터를 가져오지 못했습니다. API키 활성화 여부나 주소를 확인하세요.')
  } finally {
